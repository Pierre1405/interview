...
class VehicleOptionCountProcessorTest {
	...
	"price": 34
        ...
        val outputTopic =
            topologyTestDriver.createOutputTopic(OUTPUT_TOPIC, StringDeserializer(), StringDeserializer())
        ...
        optionInputTopic.pipeInput("1", optionJson2)
        optionInputTopic.pipeInput("2", optionJson3)
        optionInputTopic.pipeInput(null, optionJson4)

        val vehicleList: List<KeyValue<String, VehicleWithOptions>> =
            topologyTestDriver.getKeyValueStore<String, VehicleWithOptions>("vehicle_with_option_aggregation").all()
                .asSequence().toList()


        log.info(topologyTestDriver.allStateStores.keys)
